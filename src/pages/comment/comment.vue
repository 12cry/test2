<template>
  <view class="cu-list menu-avatar comment margin-top solids-top">
    <view class="cu-item">
      <view class="cu-avatar round" :style="'background-image:url('+postData.avatarUrl+');'"></view>
      <view class="content">
        <view class="text-grey">{{postData.nickName}}</view>
        <view class="text-gray text-content text-df">{{postData.content}}</view>
...
import { appreciate } from '@/api/comment';
...
export default {
    ...
    data() {
        return {
            appreciate: this.postData.appreciate
        };
    },
    created() {},
    methods: {
        toggleAppreciate() {
            ...
            appreciate({ commentId: this.postData.id, appreciate: this.appreciate });
        },
